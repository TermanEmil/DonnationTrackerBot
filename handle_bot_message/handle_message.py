...
from form_handling import resolve_form_step, RequestLanguage, DonationStatus, AboutCampaign, \
    RequestScreenshot, SharePaymentDetails
...
def handle_core(update: telegram.Update):
    ...
    if message_is_new_donation_cmd(session, update.message.text):
        session.form_step = SharePaymentDetails.__name__
    ...
    if message_is_attach_screenshot(session, update.message.text):
...
def message_is_attach_screenshot(session: Session, message: str):
    return message in ['/attach_donation']
