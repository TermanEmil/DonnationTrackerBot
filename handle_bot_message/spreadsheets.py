...
def add_to_spreadsheet_last_upload_data(session: Session):
    ...
    submissions.append_row(_build_row(session))
    print(f"Added to spreadsheets for chat_id: {session.chat_id}")


def _build_row(session: Session):
    last_upload_url = list(filter(None, session.uploads.split('\n')))[-1]
    return [
        session.contact,
